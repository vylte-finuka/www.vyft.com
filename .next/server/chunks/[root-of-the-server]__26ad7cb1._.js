...
async function handler(req, res) {
    ...
    try {
        if (action === "create") {
            console.log("Création ou récupération d'un client NowPayments...");
            // Créer un paiement via NowPayments
            const paymentResponse = await __TURBOPACK__imported__module__$5b$externals$5d2f$axios__$5b$external$5d$__$28$axios$2c$__esm_import$29$__["default"].post(`${NOWPAYMENTS_API_URL}/payment`, {
                price_amount: 10.0,
                price_currency: "EUR",
                order_id: auth0UserId,
                order_description: "Création de paiement initial",
            ...
            console.log("Paiement NowPayments créé :", paymentData.payment_id);
            ...
            console.log("Annulation d'un abonnement NowPayments...");
            // Note : NowPayments ne semble pas avoir de gestion directe des abonnements dans la spécification fournie.
            // Vous devrez gérer les abonnements manuellement ou via un autre service.
            return res.status(400).json({
                error: "La gestion des abonnements n'est pas prise en charge par NowPayments."
            });
        } else {
            console.error("Action non valide :", action);
            return res.status(400).json({
                error: "Action non valide. Utilisez 'create'."
            });
        }
    } catch (error) {
        console.error("Erreur NowPayments ou Auth0 :", error.response?.data || error.message || error);
        return res.status(500).json({
            error: "Erreur lors de la gestion du paiement ou des métadonnées Auth0."
        });
    }
}
__turbopack_async_result__();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__26ad7cb1._.js.map