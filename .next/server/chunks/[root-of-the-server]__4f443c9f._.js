...
async function vyfthealth_receive(req, res) {
    try {
        if (req.method === 'GET') {
            ...
            // Récupérer toutes les transactions depuis MongoDB
            const data = await collection.find({}).toArray();
            // Vérifier si des données existent
            if (data.length === 0) {
                return res.status(504).json({
                    error: 'Aucune donnée trouvée dans la base de données.'
                });
            }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4f443c9f._.js.map